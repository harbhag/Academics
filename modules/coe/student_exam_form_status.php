...
if(isset($_POST['student_exam_form_status_show'])  && isset($_POST['roll_no']))
{
...
echo "<center><h4>Student Form Status </h4></center>";
echo "<table class='table table-bordered striped table-condensed '><tr style='background-color:lightgrey'><th>Uni. Roll No.</th><th>Course</th><th>Branch</th><th>Student Name</td><th>Father / Mother Name</th><th>Admission Year</th><th>Fee Received Status </th><th>Student Status</th><th>LEET Status</th></tr>";	
...
//$sql="SELECT `ED_RollNo`,`SUB_CODE`,`SUB_TITLE`,`Sub_PaperID`,`Sub_Sem`,`SUB_TP`,`Ed_Int`,`Ed_Ext`,received_status, prov_nonprov, detention_status, m_code, scheme_code, Sub_Remarks,`ucentre`, `date_of_exam`, `usession`,`Regular_Reappear` FROM `ptu_subjects` WHERE `Regular_Reappear`='".$_POST['Regular_Reappear']."' and `ED_RollNo`='".$_POST['roll_no']."' AND eligibility='Y' order by `SUB_TP` DESC, `SUB_CODE` ";
$sql="SELECT `ED_RollNo`,`SUB_CODE`,`SUB_TITLE`,`Sub_PaperID`,`Sub_Sem`,`SUB_TP`,`Ed_Int`,`Ed_Ext`,received_status, prov_nonprov, detention_status, m_code, scheme_code, Sub_Remarks,`ucentre`, `date_of_exam`, `usession`,`Regular_Reappear` FROM `ptu_subjects` WHERE `ED_RollNo`='".$_POST['roll_no']."' AND eligibility='Y' order by `SUB_TP` DESC, date_of_exam ASC,   `SUB_CODE` ";
#echo $sql;
$result = mysql_query($sql) or  die(mysql_error());

echo "<table    class='table table-bordered striped table-condensed ' >

<tr style='background-color:lightgrey'><th>Sr No.</th><th >Uni Roll No.</th><th>Subject Title</th><th>Subject Code</th><th>Elective Status</th><th>Paper ID/ M code</th><th>Semester</th><th>T/P</th><th>Is Internal Appear?</th><th>Is External  Appear?</th><th>Form Received Status</th><th>Detention Status</th><th>Provisonal Status</th><th>Scheme Code</th><th>Date of exam</th><th> Session</th> <th>Centre</th><th>Regular Reappear</th><th>External Exam Attendance </th></tr>";
$x=1;
while ($row_result = mysql_fetch_array($result))
{
	...
	{$id_status='No';}
	echo "<tr><td>$x</td><td>".$row_result['ED_RollNo']."</td><td>".$row_result['SUB_TITLE']."</td><td>".$row_result['SUB_CODE']."</td><td>".$row_result['Sub_Remarks']."</td><td>".$row_result['Sub_PaperID']." / ".$row_result['m_code']."</td><td>".$row_result['Sub_Sem']."</td><td>".$row_result['SUB_TP']."</td><td>".$ed_status."</td><td>".$id_status."</td><td>".$row_result['received_status']."</td><td>".$row_result['detention_status']."</td><td>".$row_result['prov_nonprov']."</td><td>".$row_result['scheme_code']."</td>";
	if ($row_result['date_of_exam']=='0000-00-00')
	{$date_of_exam='To be informed by Department.';}
	elseif ($row_result['date_of_exam']=='0000-00-00' && $row_result['Ed_Ext']=='0')
	{$date_of_exam=' ';}
	else
	{$date_of_exam=$row_result['date_of_exam'];}
	echo "<td>".$date_of_exam."</td><td>".$row_result['usession']."</td><td>".$row_result['ucentre']."</td><td>".$row_result['Regular_Reappear']."</td><td>";
	$result_attendance = mysql_fetch_array(mysql_query("SELECT * FROM `student_external_marks` where `university_roll_no`='".$row_result['ED_RollNo']."' and `subject_code`='".$row_result['SUB_CODE']."' and `paper_id`='".$row_result['Sub_PaperID']."' ;"));
	
	echo $result_attendance['external_attendance_status']."</td></tr>";
	$x++;
}
}
