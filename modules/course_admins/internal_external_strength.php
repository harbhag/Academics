...
if(isset($_POST['strength_type']) && isset($_POST['internal_external_strength_show']))
{
	...
	else if($_POST['strength_type']=='Theory_External_Strength')
	{
		$sql_strength = "SELECT distinct concat(cc.course_name,'(',bc.branch_name,')') as Branch, `Sub_Sem` as Semester,`SUB_CODE`,`SUB_TITLE` as Subject_Title, `shift`,`aicte_rc`,`full_part_time`, count(Ed_RollNo) as Strength,`Regular_Reappear`,SUB_ID , scheme_code, Sub_PaperID as Paper_ID  FROM `ptu_subjects` ps,course_code cc,branch_code bc WHERE `SUB_TP` = 'T' AND `Ed_Ext` = '1' and `received_status`='Y' and eligibility ='Y'  and ps.course_code=cc.course_code and bc.branch_code= ps.FRM_BRID  AND ps.FRM_BRID='".$branch_code."' group by ps.FRM_BRID,`Sub_Sem`,`SUB_CODE`,`shift`,`aicte_rc`,`full_part_time`,`Regular_Reappear`, scheme_code order by ps.course_code,ps.FRM_BRID,`Sub_Sem`,`SUB_CODE`,`Regular_Reappear`,`shift`,`aicte_rc`,`full_part_time`";
			
		$sql_strength_count ="SELECT count(Ed_RollNo) as total_strength   FROM `ptu_subjects` ps WHERE `SUB_TP` = 'T' AND `Ed_Ext` = '1' and `received_status`='Y' and eligibility ='Y' AND ps.FRM_BRID='".$branch_code."'  ";
	}
	...
	<th>Subject Code</th>
	<th>Subject Title</th>";
	if($_POST['strength_type']=='Theory_External_Strength')
	{
	echo "<th>Scheme Code</th>";
	}	
	echo "<th>Shift</th>
	<th>AICTE</th>
	...
	if($_POST['strength_type']!='Theory_External_Strength')
	{
	echo "<th>Filled Strength</th>";
	}
echo "<th>Regular / Reappear
	<!--<th>Remarks</th>-->	";
	if ($_SESSION['usertype']=='result_team' && $_POST['strength_type']!='Theory_External_Strength' )
	{
		echo "<th>Received</th>";
	}
	$x=1;
	while($row_result = mysql_fetch_array($result_strength_list))
		{
			...
			{
				$sql_strength_filled = " SELECT count(sim.university_roll_no) as filled_strength  FROM `student_external_marks` sim WHERE sim.`theory_practical` = 'T'   AND
				 sim.branch_code='".$branch_code."' AND
				sim.subject_code='".$row_result['SUB_CODE']."' AND
				sim.shift='".$row_result['shift']."' AND
				sim.aicte_rc='".$row_result['aicte_rc']."' AND
				...
				$sql_strength_filled_count = "SELECT count(sim.university_roll_no) as filled_strength_count  FROM `student_external_marks` sim WHERE sim.`theory_practical` = 'T'  AND	 sim.branch_code='".$branch_code."' ;";
			...
			<td>".$row_result['Subject_Title']."</td>";
			 if($_POST['strength_type']=='Theory_External_Strength')
			{
			echo "<td>".$row_result['scheme_code']."</td>";
			}
			echo "<td>".$row_result['shift']."</td>
			<td>".$row_result['aicte_rc']."</td>
			<td>".$row_result['full_part_time']."</td>
			<td>".$row_result['Strength']."</td>";
			if($_POST['strength_type']!='Theory_External_Strength')
			{
			echo	"<td>".$row_strength_filled['filled_strength']."</td>";
			}
			echo "<td>".$row_result['Regular_Reappear']."</td>";
			#############################
			#Received ON
			#############################
			if ($_SESSION['usertype']=='result_team' && $_POST['strength_type']!='Theory_External_Strength')
	...
	{	
		...
		*/
		...
		<span style='font-weight:bold'>Program </span></td><td>
          <select name='course_branch' id='course_branch' class='input-xxlarge'>";
		...
		<option value='Practical_Internal_Strength'>Practical Internal Strength</option>
		<option value='Practical_External_Strength'>Practical External Strength</option>
		<option value='Theory_External_Strength'>Theory External Strength</option>";
		echo "</select></td></tr>
		<tr><td></td><td>	
		<input type='hidden' name='internal_external_strength_show' value='internal_external_strength_show' /> 
		<input type='submit' name='submit' class='btn btn-danger' value='Submit'></form></td></tr></table>";
