	...
	$n=$_POST['num'];$x=1;
		for($i=1;$i<=$n;$i++)
		{
			...
			if($_POST['add_subject'.$x]=='1')
			{
				...
				if($_POST['theory_practical'.$x]=='T')
				{$lecture_type='L';}
				if($_POST['theory_practical'.$x]=='P')
				{$lecture_type='P';}
				if($_POST['theory_practical'.$x]=='TU')
				{$lecture_type='T';}
				
				$sql_time_table="INSERT INTO `time_table`(`scheme_master_id`, `course_code`, `branch_code`, `paper_id`, `m_code`, `subject_code`, `subject_title`, `theory_practical`, `lecture_type`, `elective_details`, `semester`, `full_part_time`, `sessional_max_marks`, `ssection`, `sgroup`, `teacher_username`, `aicte_rc`, `regular_reappear`, `shift`, scheme_code, allotment_timestamp,alloted_username,contact_hours,teacher_type) VALUES ('".$row_scheme_master['scheme_master_id']."', '".$row_scheme_master['course_code']."', '".$row_scheme_master['branch_code']."', '".$row_scheme_master['paper_id']."', '".$row_scheme_master['m_code']."', '".$row_scheme_master['subject_code']."', '".$row_scheme_master['subject_title']."', '".$_POST['theory_practical'.$x]."', '".$lecture_type."', '".$row_scheme_master['elective_details']."', '".$row_scheme_master['semester']."', '".$row_scheme_master['full_part_time']."', '".$sessional_max_marks."', '".$_POST['ssection'.$x]."', '".$_POST['sgroup'.$x]."', '".$_POST['teacher_username'.$x]."', '".$row_scheme_master['aicte_rc']."', 'Regular', '".$_POST['shift'.$x]."', '".$_POST['scheme_code'.$x]."','".date('Y-m-d H:i:s')."','".$_SESSION['username']."','".$_POST['contact_hours'.$x]."','".$_POST['teacher_type'.$x]."') ;";
			#echo $sql_time_table;
...
else if (isset($_POST['time_table_subject_allotment_show']))
{
	...
	{
	$sql_subject = "SELECT * from scheme_master where  branch_code='".$_POST['Branch_Name']."' and semester='".$_POST['Semester']."' and scheme_code='".$_POST['Scheme_Code']."' and full_part_time='".$_POST['Full_/_Part_Time']."' and  aicte_rc='".$_POST['AICTE_/_RC']."' and L!='0' order by subject_code ASC ;";
	#echo $sql_subject;
	}
	if($_POST['theory_practical']=='P')
	{
	$sql_subject = "SELECT * from scheme_master where  branch_code='".$_POST['Branch_Name']."' and semester='".$_POST['Semester']."' and scheme_code='".$_POST['Scheme_Code']."' and full_part_time='".$_POST['Full_/_Part_Time']."' and  aicte_rc='".$_POST['AICTE_/_RC']."' and P!='0' order by subject_code ASC ;";
	}
	if($_POST['theory_practical']=='TU')
	{
	$sql_subject = "SELECT * from scheme_master where  branch_code='".$_POST['Branch_Name']."' and semester='".$_POST['Semester']."' and scheme_code='".$_POST['Scheme_Code']."' and full_part_time='".$_POST['Full_/_Part_Time']."' and  aicte_rc='".$_POST['AICTE_/_RC']."' and T!='0' order by subject_code ASC ;";
	}
	#echo $sql_subject;
	...
	while ($row_subject = mysql_fetch_array($result_subject))
	{
		...
		echo "<input type='hidden' name='theory_practical$x' value=".$_POST['theory_practical'].">";
