...
show_label('info','Assignment Module');
echo "<br/>";
...
`status`,internal_lock_status,grace_period_allowed,status_daily_attendance,status_aggregate_attendance,elective_details FROM `time_table` 
WHERE 
`status` = '1' AND 
status_assignments='1' AND
course_code!='3' AND
...
<tr class='warning'>
	...
	<td><?php upload_assignments_marks($row['course_code'],$row['branch_code'],$row['subject_code'],$row['paper_id'],
	...
	<td><?php update_assignments_marks($row['course_code'],$row['branch_code'],$row['subject_code'],$row['paper_id'],
	$row['subject_title'],$row['theory_practical'],$row['semester'],$row['shift'],
	$row['full_part_time'],$row['aicte_rc'],$row['exam_month'],$row['exam_year'],$row['ssection'],$row['sgroup'],$count,$row['internal_lock_status'],$row['elective_details']); ?></td>
	
	<td><?php details_assignments_marks($row['course_code'],$row['branch_code'],$row['subject_code'],$row['paper_id'],
	$row['subject_title'],$row['theory_practical'],$row['semester'],$row['shift'],
