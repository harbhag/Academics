...
show_label('info','Aggregate Attendance Record');
echo "<br/>";

$subject_allotment = mysql_query("SELECT DISTINCT id,course_code,branch_code,subject_code,paper_id,aicte_rc,shift,full_part_time,ssection,sgroup,semester,subject_title,theory_practical,start_date,end_date,total_lectures,exam_month,exam_year FROM
aggregate_attendance_student WHERE teacher_username='".$_SESSION['username']."' AND backup='0' AND theory_practical='P'") or die(mysql_error());

if(mysql_num_rows($subject_allotment)==0) {
	show_label('info','No Record Found for Aggregate Attendance (Practical)');
	exit();
}
?>
