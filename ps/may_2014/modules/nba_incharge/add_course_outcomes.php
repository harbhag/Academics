...
$scheme_master = mysql_query("SELECT *  from scheme_master  where semester='".$_POST['semester']."' and scheme_code='".$_POST['scheme_code']."' and branch_code='".$course_branch['1']."'; ");
...
<center>
	...
	<form method='post' action=''>
          ...
          while($row = mysql_fetch_assoc($scheme_master)) {
				...
				<input type='hidden' name='branch_code' id='branch_code'   value='".$course_branch['1']."' />
				<input type='hidden' name='scheme_code' value='".$_POST['scheme_code']."' />
				
				<tr><td>
					<input type='hidden' name='add_course_outcomes_detail' value='add_course_outcomes_detail' />
					<button type='submit' name='submit' class='btn btn-large btn-danger'>Submit</button>
					</tr></td></form></table>";
