...
{	
	...
	$qmm_sql = mysql_query("SELECT * FROM internal_question_structure WHERE ass_tool='P' AND course_code='".$row_sm['course_code']."'") or die(mysql_query());
 ...
 <input type='hidden' name='course_code' value='".$row_sm['course_code']."' />
				...
				<input type='hidden' name='ip_no' value='".$_POST['ip_no']."' />
				<input type='hidden' name='total_questions' value='".$total_qcount."' />
				<input type='hidden' name='ass_tool' value='P' />
	...
	<input type='hidden' name='add_internal_practical_co_que_mapping_details' value='add_internal_practical_co_que_mapping_details' />";
