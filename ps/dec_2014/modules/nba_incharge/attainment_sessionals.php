<?php 
	...
	while($sc = mysql_fetch_assoc($subject_codes)) {
		...
		for($i=1;$i<=3;$i++) {
			...
			$total_questions = mysql_query("SELECT * FROM internal_question_structure WHERE ass_tool='S' AND course_code='".$course_branch[0]."'") or die(mysql_query());
			...
			$total_marks_obt_sql = mysql_query("SELECT SUM(".$question_id.") as obt FROM student_sessionals_record WHERE 
			branch_code='".$course_branch[1]."' AND
			course_code='".$course_branch[0]."' AND
			backup='0' AND
			attendance_status='Present' AND
			subject_code='".$sc['subject_code']."' AND
			sessional_no='".$i."'") or die(mysql_error());
			
			
			$total_marks_max_sql = mysql_query("SELECT SUM(".$mquestion_id.") maxm FROM student_sessionals_record WHERE 
			branch_code='".$course_branch[1]."' AND
			course_code='".$course_branch[0]."' AND
			backup='0' AND
			attendance_status='Present' AND
			...
			$semester_sql = mysql_query("SELECT DISTINCT semester FROM student_sessionals_record WHERE 
			branch_code='".$course_branch[1]."' AND
			course_code='".$course_branch[0]."' AND
			backup='0' AND
			subject_code='".$sc['subject_code']."' AND
			...
			while($pci = mysql_fetch_assoc($program_cos_id_sql)) {
				...
				subject_code='".$sc['subject_code']."' AND
				sessional_assign_no='".$i."'") or die(mysql_error());
