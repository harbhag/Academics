<?php 
	...
	while($sc = mysql_fetch_assoc($subject_codes)) {
		...
		$total_questions = mysql_query("SELECT * FROM internal_question_structure WHERE ass_tool='A' AND course_code='".$course_branch[0]."'") or die(mysql_query());
		...
		while($tq = mysql_fetch_assoc($total_questions)) {
			...
			$total_marks_obt_sql = mysql_query("SELECT SUM(".$question_id.") as obt FROM student_assignment_record WHERE 
			branch_code='".$course_branch[1]."' AND
			course_code='".$course_branch[0]."' AND
			backup='0' AND
			subject_code='".$sc['subject_code']."' AND
			assignment_no='".$i."'") or die(mysql_error());
			$total_marks_max_sql = mysql_query("SELECT SUM(".$mquestion_id.") maxm FROM student_assignment_record WHERE 
			branch_code='".$course_branch[1]."' AND
			course_code='".$course_branch[0]."' AND
			backup='0' AND
			subject_code='".$sc['subject_code']."' AND
			...
			$semester_sql = mysql_query("SELECT DISTINCT semester FROM student_assignment_record WHERE 
			branch_code='".$course_branch[1]."' AND
			course_code='".$course_branch[0]."' AND
			backup='0' AND
			subject_code='".$sc['subject_code']."' AND
