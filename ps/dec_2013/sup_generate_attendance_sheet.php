...
$htm[] = "<center><table cellpadding='4' width='100%'><tr>
			...
			<br/><span style='font-size:18px'>Affiliated to Punjab Technical University</span>
			<br/><span style='font-size:14px'>END SEMESTER EXAMINATIONS, April-May, 2014</span>
		...
		<tr>
		<td align='center' style='font-size:16px'>Centre No:<b>".$_POST['ucentre']."</b> Session: <b>".$_POST['usession']."</b> Course (B.Tech./M.Tech./MBA/MCA/Ph.D.):<b>".$course_name['course_name']."</b>FT/PT:<b>".$_POST['full_part_time']."</b>AICTE/RC:<b>".$_POST['aicte_rc']."</b> Branch:<b>".$branch_name['branch_name']."</b> Semester:<b>".$_POST['semester']."</b> Regular/ Reappear:<b>".$_POST['regular_reappear']."</b> Total Candidates:<b>".mysql_num_rows($rollno_details)."</b></td>
		</tr>
	  </table></center>";
...
while($row=mysql_fetch_assoc($rollno_details)) {
	...
	if($page_count==$limit) {
		...
		<tr>
		<td colspan='4'>Total Present<br/></td>";
		for($i=1;$i<=$sub_count;$i++) {
				$htm[] = "<td><br/></td>";
		}
		...
		<tr>
		<td colspan='4' height='20'>Signature of Invigilator/s & (Room No.)</td>";
		for($j=1;$j<=$sub_count;$j++) {
				$htm[] = "<td></td>";
		}
		
		$col_count = 4+$sub_count;
...
<tr>
<td colspan='4' height='20'>Signature of Invigilator/s & (Room No.)<br/></td>";
for($i=1;$i<=$sub_count;$i++) {
		$htm[] = "<td><br/></td>";
}
