...
Following students of previous semesters (who were detained earlier in ".$th_pr." ) are eligile to appear for End Semester Examinations, April-May, 2014  as per details given below:-<br/><br/><br/>";
...
while($row = mysql_fetch_assoc($rolls)) {
	...
	$branch = fetch_resource_db('branch_code',array('branch_code'),array($row['branch_code']),'resource_array_value','branch_name');
		...
		if($count==$entries_per_page) {
			...
			<td align='right' style='font-size:12px'>(".$hod.")
			<br/>HOD
			</td>
