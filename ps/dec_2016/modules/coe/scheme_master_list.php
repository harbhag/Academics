...
if(isset($_POST['scheme_master_list_show'])) 
{
	...
	<tr style='background-color:lightgrey'><th>Sr No.</th><th>Course Code</th><th>Branch Code</th><th>Subject Title</th><th>Subject Code</th><th>Paper ID </th><th>M Code</th><th>Sem.</th><th>T/P</th><th>Elective Type</th><th>Int. Marks</th><th>Ext. Marks</th><th>Credits</th><th>Scheme Code</th><th>Full /Part Time </th><th>AICTE / RC</th><th>Subject Group</th><th> Six Month Trg.</th><th>LEET Subject (Disabled) </th></tr>";

	$x=1;
	while($row_sm = mysql_fetch_assoc($sm_list))
	{	
	echo "<tr><td>$x</td><td>".$row_sm['course_code']."</td><td>".$row_sm['branch_code']."</td><td>".$row_sm['subject_title']."</td><td>".$row_sm['subject_code']."</td><td>".$row_sm['paper_id']."</td><td>".$row_sm['m_code']."</td><td>".$row_sm['semester']."</td><td>".$row_sm['theory_practical']."</td><td>".$row_sm['elective_details']."</td><td>".$row_sm['internal_max_marks']."</td><td>".$row_sm['external_max_marks']."</td><td>".$row_sm['credit']."</td><td>".$row_sm['scheme_code']."</td><td>".$row_sm['full_part_time']."</td><td>".$row_sm['aicte_rc']."</td><td>".$row_sm['subject_group']."</td><td>".$row_sm['six_month_training']."</td><td>".$row_sm['leet_subject_status_disable']."</td></tr>";
	$x++;
	}
