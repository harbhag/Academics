...
show_label('info','Internal Practical Marks');
echo "<br/>";
...
if($_POST['elective_details']=='Compulsory') {
	$roll_nos = mysql_query("SELECT distinct university_roll_no FROM student_internal_practical_record WHERE
	...
	$roll_nos = mysql_query("SELECT distinct university_roll_no FROM student_internal_practical_record WHERE
		...
		student_internal_practical_record.university_roll_no IN 
...
while($row = mysql_fetch_assoc($roll_nos)) {
	...
	$autoid_sql = mysql_query("SELECT distinct autoid FROM student_internal_practical_record WHERE
		course_code = '".$_POST['course_code']."' AND
		branch_code = '".$_POST['branch_code']."' AND
		shift = '".$_POST['shift']."' AND
		subject_code='".$_POST['subject_code']."' AND
	...
	$student_details = fetch_resource_db('student_info',array('university_roll_no'),array($row['university_roll_no']),'resource_array','');
		...
		echo "<tr class='warning'>
			...
			while($rows = mysql_fetch_assoc($total_questions)) {
				...
				$internal_practical_marks_sql = mysql_query("SELECT distinct ".$dquestion_id." FROM student_internal_practical_record WHERE
				course_code = '".$_POST['course_code']."' AND
				branch_code = '".$_POST['branch_code']."' AND
				shift = '".$_POST['shift']."' AND
				subject_code='".$_POST['subject_code']."' AND
				...
				$internal_practical_marks = mysql_fetch_assoc($internal_practical_marks_sql);
				...
				echo "<input class='input-small' size='16' type='hidden' value='".$internal_practical_marks[$dquestion_id]."' id='p_q$question_id' name='p_q$question_id'>";
			
			
				echo "<td><input class='input-small' size='16' value='".$internal_practical_marks[$dquestion_id]."' type='text' id='q$question_id' name='q$question_id'>
				<script>
...
<input type='hidden' name='teacher_update_internal_practical_marks' value='<?php echo $count; ?>' />
