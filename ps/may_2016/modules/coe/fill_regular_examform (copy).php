...
$check_ps=mysql_query(" Select * from ptu_subjects where full_part_time='".$_POST['full_part_time']."' and aicte_rc='".$_POST['aicte_rc']."' and  `Sub_Sem`='".$_POST['semester']."' and course_code='".$course_branch[0]."' and `FRM_BRID`='".$course_branch[1]."'; ");
	
	$ps_rows = mysql_num_rows($check_ps);
	if ($ps_rows!='0')
	{
		show_label('warning','Record Already Updated');
	}
	else
	{
	$student_list = mysql_query("SELECT * FROM student_info where student_status='Onroll' and full_part_time='".$_POST['full_part_time']."' and aicte_rc='".$_POST['aicte_rc']."' and  semester='".$_POST['semester']."' and course_code='".$course_branch[0]."' and branch_code='".$course_branch[1]."' and fee_paid_status='Y' and eligibility='Y' and stuck_off_status='N'; ");
	...
	{	
	 	...
	 	if (($_POST['semester']=='1' || $_POST['semester']=='2') &&  $course_branch[0]=='1')
		{
			...
			`SUB_TP`, `Sub_PaperID`, `Sub_Sem`, `ED_RollNo`, `FRM_BRID`, `StudentName`,`full_part_time`,`eligibility`,`course_code`,`shift`,`aicte_rc`,`FRM_BATCH`,`Sub_Remarks`,`Regular_Reappear`,`student_status`,father_name,mother_name,m_code,batch,scheme_code)
			select sm.subject_code,sm.subject_title,
			sm.theory_practical,sm.paper_id,si.semester,si.university_roll_no,si.branch_code,si.ptu_student_name, si.full_part_time,
			si.eligibility, si.course_code, si.shift, si.aicte_rc, si.admission_year, sm.elective_details,'Regular',si.student_status,si.ptu_father_name,si.ptu_mother_name,sm.m_code,si.batch,sm.scheme_code from
			scheme_master sm,student_info si, student_subject_group ssg  WHERE sm.branch_code=si.branch_code and sm.course_code=si.course_code and si.semester=sm.semester  and si.scheme_code_first_year=sm.scheme_code  and ssg.subject_group=sm.subject_group and si.university_roll_no='".$row_student['university_roll_no']."' and si.university_roll_no=ssg.university_roll_no ; ";
		}
		else
		{
			...
			{				
				$insert_sql ="INSERT INTO `ptu_subjects` (`SUB_CODE`, `SUB_TITLE`,
				`SUB_TP`, `Sub_PaperID`, `Sub_Sem`, `ED_RollNo`, `FRM_BRID`, `StudentName`,`full_part_time`,`eligibility`,`course_code`,`shift`,`aicte_rc`,`FRM_BATCH`,`Sub_Remarks`,`Regular_Reappear`,`student_status`,father_name,mother_name,m_code,batch,scheme_code)
				select sm.subject_code,sm.subject_title, sm.theory_practical,sm.paper_id,si.semester,si.university_roll_no,si.branch_code,si.ptu_student_name,si.full_part_time,si.eligibility, si.course_code, si.shift, si.aicte_rc, si.admission_year,
				sm.elective_details,'Regular',si.student_status,si.ptu_father_name,si.ptu_mother_name,sm.m_code,si.batch,sm.scheme_code 
				from scheme_master sm,student_info si ";
				
				if ($row_subject['elective_details']!='Compulsory')
				{	
				$insert_sql .= ", student_elective_subjects ses ";
				}
			
				$insert_sql .= "WHERE sm.scheme_master_id='".$row_subject['scheme_master_id']."' and 
					sm.branch_code=si.branch_code and sm.course_code=si.course_code and sm.scheme_code = '".$row_student['scheme_code_branch']."' and si.semester=sm.semester  and si.scheme_code_branch=sm.scheme_code   and si.university_roll_no='".$row_student['university_roll_no']."' ";
				...
				if ($row_subject['elective_details']!='Compulsory')
				{	
				 $insert_sql .=  " and ses.elective_details=sm.elective_details and ses.m_code=sm.m_code and ses.paper_id=sm.paper_id and ses.university_roll_no='".$row_student['university_roll_no']."' " ;
				}
