...
function get_internal_marks_csv() {
	...
	if($_POST['internal_external']=='I') {
		...
		$select_fields = "university_roll_no,upper(student_name),subject_code,semester,theory_practical,regular_reappear,internal_max_marks,internal_obtained_marks";
		...
		$col_name = get_tables_cols('student_internal_marks',array('university_roll_no','student_name','subject_code','semester','theory_practical','regular_reappear','internal_max_marks','internal_obtained_marks'));
	}
	...
	//	$select_fields = "university_roll_no,upper(student_name),subject_code,semester,theory_practical,regular_reappear,external_max_marks,external_obtained_marks,grading_type,normalized_marks,grade_letter,mean,std_dev,total_students";
		$select_fields = "university_roll_no,upper(student_name),subject_code,semester,theory_practical,regular_reappear,external_max_marks,external_obtained_marks";
	...
	//	$col_name = get_tables_cols('student_internal_marks',array('university_roll_no','student_name','subject_code','semester','theory_practical','regular_reappear','external_max_marks','external_obtained_marks','grading_type','normalized_marks','grade_letter','mean','std_dev','total_students'));
		$col_name = get_tables_cols('student_internal_marks',array('university_roll_no','student_name','subject_code','semester','theory_practical','regular_reappear','external_max_marks','external_obtained_marks'));
	}
	...
	while($row=mysql_fetch_array($data)) {
		...
		$sql_cbs = mysql_query("select * from student_info where university_roll_no='".$row[0]."' ;");
			$cbs= mysql_fetch_assoc($sql_cbs);
			if( $cbs['cbs']=='')
			{
			$sql_cbs = mysql_query("select * from student_info_ex where university_roll_no='".$row[0]."' ;");
			$cbs= mysql_fetch_assoc($sql_cbs);	
			}
	...
	* */
	...
	if($_POST['internal_external']=='I') {
		...
		while($row = mysql_fetch_assoc($data)) {
			...
			$sql_cbs = mysql_query("select * from student_info where university_roll_no='".$uroll_array[1]."' ;");
			$cbs= mysql_fetch_assoc($sql_cbs);
			if( $cbs['cbs']=='')
			{
			$sql_cbs = mysql_query("select * from student_info_ex where university_roll_no='".$uroll_array[1]."' ;");
			$cbs= mysql_fetch_assoc($sql_cbs);	
			}
		if( $cbs['cbs']=='Yes')
		{
		...
		if( $cbs['cbs']=='Yes')
		{
			...
			) 
			{
					...
					if($_POST['theory_practical']=='T' or $_POST['theory_practical']=='P') {
						$mysql_query_1 = "UPDATE student_internal_marks set 
						...
						if($_POST['theory_practical']=='T')
						{
								 $mysql_query_1 .= " paper_id='".$_POST['paper_id']."'AND";
						}
						
						$mysql_query_1 .= " teacher_username='".$_SESSION['username']."' ";
						
						mysql_query($mysql_query_1) or die(mysql_error());
						#echo $mysql_query_1;
					}
