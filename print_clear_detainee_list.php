...
detained_status='N' AND
c_exam_month='".$c_month."' AND
c_exam_year='".$ey."' AND
...
Following students of previous semesters (who were detained earlier in ".$th_pr." ) are eligile to appear for End Semester Examinations, ".$exam_session."  as per details given below:-<br/><br/><br/>";
...
while($row = mysql_fetch_assoc($rolls)) {
	...
	$branch = fetch_resource_db('branch_code',array('branch_code'),array($row['branch_code']),'resource_array_value','branch_name');
		...
		if(mysql_num_rows($student_data_si_sql)==0) {
			
			$student_data_si_sql = mysql_query("SELECT * FROM student_info_ex WHERE 
			university_roll_no='".$row['university_roll_no']."'
			") or die(mysql_error());
		}
		...
		<td>".strtoupper($row['sname'])." / ".strtoupper($row['fname'])."</td>
		<td>".$row['semester']."</td>
		...
		<td>".$row['c_exam_month'].", ".$row['c_exam_year']."</td>
		</tr>";
	...
	<td>Note:
		...
		<br/>2. B.Tech.(1st Year) students must also be considered while submitting detention clearance record.
	</td>
	</tr>
	</table>";
...
$dompdf->render();
$dompdf->stream("Cleared Detainee List_".$_POST['theory_practical']."_".date('d-m-Y H:i:s').".pdf");
